...
<div class="mt-5">
  <h3 class="py-3">Edit video</h3>

      <form class="form-group" action="{{route('video.update', $video->id)}}" method="post" enctype="multipart/form-data">
        @csrf

            <div class="mt-4">
              <label for="video_title">Video Title</label>
              <input type="text" name="video_title" id="video_title" class="form-control" value="{{$video->video_title}}">
            ...
            <div class="mt-4">
              ...
              <textarea name="video_description" rows="8" cols="80" class="form-control" id="video_description"> {{$video->video_description}}</textarea>
              @error('video_description')<div class="alert alert-danger">{{ $message }}</div>@enderror
            </div>

            <div class="mt-4">
              <label for="video_url">Video Url</label>
              <input type="text" name="video_url" id="video_url" class="form-control" value="{{$video->video_url}}">
